...
using AXAMansard.Framework.AuditTrail.Enums;
...
namespace AXAMansard.Framework.Utility
{
    public class BinarySerializer
    {
        ...
        {
            return AXAMansard.Framework.Utility.BinarySerializer.SerializeObject(ConvertToList(objectBefore, objectAfter, currentDepth, maxDepth));
        }
        ...
        public static List<TrailItem> ConvertToList(object objectBefore, object objectAfter, int currentDepth, int maxDepth)
        {
            ...
            foreach (PropertyInfo propertyInfo in dataTypeCheck.GetType().GetProperties())
            {
                ...
                else if (propertyType.IsEnum)
                {
                    if (!ignoreBeforeText && propertyBefore != null)
                    {
                        strBefore = AXAMansard.Framework.Utility.EnumBinder.SplitAtCapitalLetters(Convert.ToString(propertyBefore));
                    }
                    if (!ignoreAfterText && propertyAfter != null)
                    {
                        strAfter = AXAMansard.Framework.Utility.EnumBinder.SplitAtCapitalLetters(Convert.ToString(propertyAfter));
                    }
                }
        ...
        private static string GetValue(Type propertyType, object property)
        {
            ...
            else if (propertyType.IsEnum)
            {
                if (property != null)
                {
                    return AXAMansard.Framework.Utility.EnumBinder.SplitAtCapitalLetters(Convert.ToString(property));
                }
            }
            else
            {
